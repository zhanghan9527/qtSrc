...
//C_USBHOST_PIN is output mode for H switch
 ...
 //GPIO  USB Host  Power control
  ...
  if (!s_usbd_pin)
  {
    ...
    // get usb mode initial value.
    for(i=0; i<65; i++) {
 	    s_usbmode_check_pin = storage_debouce(s_usbmode_check_pin, C_MODE_PIN, 2);
    }
#elif (USBH_DETECTION_MODE == USBH_GPIO_D )       
  if (s_usbd_pin)
     gpio_write_io(C_USBHOST_PWR_PIN,0);
...
{
 return  1;//gpio_read_io(C_WP_D_PIN) ;
}
#if(USBH_DETECTION_MODE ==USBH_SWITCH_D)
void storage_H_D_switch(INT32U ifH)
{
...
// s_usbh_dpdm = (INT8U) USB_Host_Signal();  
  ...
  if (DP_cnt >500)
    ...
    usbh_cd =0;      
#elif (USBH_DETECTION_MODE == USBH_GPIO_D) || (USBH_DETECTION_MODE == USBH_GPIO_D_1) || (USBH_DETECTION_MODE == USBH_GPIO_D_2)
