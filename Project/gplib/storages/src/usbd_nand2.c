...
///*
	...
	{//此时buffer addr为null，使用ifwait为0时的buffer addr
		...
		#if _DRV_L1_CACHE        
        cache_drain_range((INT32U) BufferPtr_2, (INT32U) (seccount<<9)); // drain things nand driver ever touch by cpu
        cache_invalid_range((INT32U)BufferPtr_2, (INT32U)(seccount<<9)); // inval so that usb always get from dram
		#endif
		StartSectorNum_2 += seccount;
	}//*/
 return sts;
...
///*
	...
	{//此时buffer addr为null，使用ifwait为0时的buffer addr
		...
		#if _DRV_L1_CACHE        
        cache_drain_range((INT32U) BufferPtr_2, (INT32U) (seccount<<9)); // if the buffer being touch by cpu, drain it
        cache_invalid_range((INT32U)BufferPtr_2, (INT32U)(seccount<<9)); // inval so that nand driver always get from dram,
        														 // when usbd BOT_DMA_RAM_BUF_ORDER is 0, this inval must apply
		#endif
		StartSectorNum_2 += seccount;
	}//*/
