...
CODEC_START_STATUS video_decode_start(VIDEO_ARGUMENT arg, MEDIA_SOURCE src)
{
	...
	//scaler
	vid_dec_get_size(&width, &height);
	p_vid_dec_para->xoffset = 0;
	p_vid_dec_para->yoffset = 0;
	switch(arg.bScaler)
	{
	case 0:
	...
	case 3:
		p_vid_dec_para->scaler_flag = p_vid_dec_para->user_scaler_flag = C_NO_SCALER_CENTER;
		vid_dec_set_scaler(C_NO_SCALER_CENTER, video_output_format, width, height, arg.DisplayBufferWidth, arg.DisplayBufferHeight);
		p_vid_dec_para->xoffset = ((arg.DisplayBufferWidth - width) / 2) & (~0x0F);
		p_vid_dec_para->yoffset = ((arg.DisplayBufferHeight - height) / 2) & (~0x0F);
		break;
...
VIDEO_CODEC_STATUS video_decode_get_nth_video_frame(VIDEO_INFO *video_info, VIDEO_ARGUMENT arg, MEDIA_SOURCE src, INT32U nth_frame)
{
	...
	//img_scale.input_buf1=(void*)(p_vid_dec_para->video_decode_addr[0]);
	//image_scale_to_buff();
	//Turn_PhotoH((INT32U)p_vid_dec_para->video_decode_addr[0]);
	Video_view((INT32U)p_vid_dec_para->video_decode_addr[0],2);
