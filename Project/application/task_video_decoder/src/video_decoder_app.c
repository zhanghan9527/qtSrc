...
INT32S scaler_size_start(scaler_info *pScaler)
{
    ...
    case C_NO_SCALER_CENTER:
    	if((pScaler->input_x <= pScaler->output_x) && (pScaler->input_y <= pScaler->output_y)) {
			scaler_output_pixels_set(1<<16, 1<<16, pScaler->output_buffer_x, pScaler->output_y);
    	}
     	break;
...
void mjpeg_decode_set_scaler(mjpeg_info *pMjpeg)
{
    ...
    case C_NO_SCALER_FIT_BUFFER:
      	if((pMjpeg->jpeg_valid_w <= pMjpeg->jpeg_output_w) && (pMjpeg->jpeg_valid_h <= pMjpeg->jpeg_output_h)) {
    ...
    case C_NO_SCALER_CENTER:
    	if((pMjpeg->jpeg_valid_w <= pMjpeg->jpeg_output_w) && (pMjpeg->jpeg_valid_h <= pMjpeg->jpeg_output_h)) {
      		scaler_output_pixels_set(1<<16, 1<<16, pMjpeg->jpeg_output_buffer_w, pMjpeg->jpeg_valid_h);
      	}
    	break;
