...
/* function */
void capture_task_entry(void *parm);
...
#if (AVI_AUDIO_ENCODE_EN == 1) && (C_USB_AUDIO == CUSTOM_ON)
	// start audio 
	if(pAviEncAudPara->audio_format && ((avi_encode_get_status()&C_AVI_ENCODE_AUDIO) == 0)) {
		if(avi_audio_record_start() < 0) {
			RETURN(STATUS_FAIL);	
...
void capture_task_entry(void *parm)
{
	...
	if(avi_encode_get_status()&C_AVI_ENCODE_SENSOR) {
		avi_encode_clear_status(C_AVI_ENCODE_SENSOR);
		POST_MESSAGE(AVIEncodeApQ, MSG_AVI_STOP_SENSOR, avi_encode_ack_m, 5000, msg, err);	
		DEBUG_MSG(DBG_PRINT("a.sensor stop.\r\n")); 
	}
	
	// change sensor resolution
	DEBUG_MSG(DBG_PRINT("b.sensor change size.\r\n")); 
	avi_encode_eof_isr_register(capture_eof_isr);
	nRet = video_encode_sensor_start(cap->width, cap->height, cap->csi_buf, cap->csi_buf);
	if(nRet < 0) {
		RETURN(STATUS_FAIL);
	} 

	// get jpeg header
	jpeg_header_generate(cap->quality, cap->width, cap->height);
	gp_memcpy((INT8S*)cap->jpeg_buf, (INT8S*)jpeg_422_header, sizeof(jpeg_422_header)); 
		 
	//wait sensor ready
	DEBUG_MSG(DBG_PRINT("c.wait frame buffer ready\r\n")); 
...
Return:
	...
	if((avi_encode_get_status()&C_AVI_ENCODE_SENSOR) == 0) {
		...
		POST_MESSAGE(AVIEncodeApQ, MSG_AVI_START_SENSOR, avi_encode_ack_m, 5000, msg, err);	
		avi_encode_set_status(C_AVI_ENCODE_SENSOR);
		DEBUG_MSG(DBG_PRINT("d.sensor recovery size\r\n")); 
	}
	...
	if(encflag) {
		...
		DEBUG_MSG(DBG_PRINT("jpeg encode size = %d\r\n", cap->jpeg_len)); 
	}

	if(nRet >= 0) {
		OSMboxPost(cap->Ack_m, (void*)C_ACK_SUCCESS);	
	} else {
		OSMboxPost(cap->Ack_m, (void*)C_ACK_FAIL);	
	}
	OSTaskDel(OS_PRIO_SELF);
}