...
static void (*pfunc_eof_isr)(void);
...
static INT32S sensor_mem_alloc(void)
{
	...
	buffer_size = pAviEncVidPara->sensor_capture_width * (pAviEncVidPara->sensor_capture_height + 0) << 1;
...
void avi_encode_eof_isr_register(void (*pfunc)(void))
{
	if(pfunc) {
		pfunc_eof_isr = pfunc;
	}
