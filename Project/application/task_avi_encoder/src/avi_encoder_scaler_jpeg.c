...
#endif
#if APP_FACE_DETECTION_EN == 1
	ObjDetResult_t *result, draw;
	INT32S ret;
	
	gp_memset((INT8S *)&draw, 0x00, sizeof(ObjDetResult_t));
#endif
	while(1)
	{
		...
		default:
			...
			if(pAviEncVidPara->dispaly_scaler_flag == 0) {
			...
			#endif
			#if APP_FACE_DETECTION_EN == 1					
				ret = object_detect_set_frame(display_frame, 
											pAviEncVidPara->display_buffer_width, 
											pAviEncVidPara->display_buffer_height, 
											BITMAP_YUYV);
				if(ret >= 0) {
					result = object_detect_wait_frame_finish(0);
					if(result) {
						gp_memcpy((INT8S *)&draw, (INT8S *)result, sizeof(ObjDetResult_t));
					} 
					obj_detect_show_result(display_frame, 
											pAviEncVidPara->display_buffer_width,
											pAviEncVidPara->display_buffer_height,
											&draw);
				}
				...
				if(display_frame == 0) {
					DEBUG_MSG(DBG_PRINT("display_frame = 0\r\n"));
					goto DEFAULT_END;
				}
				...
				Encode_frame=scaler_frame;
		    	Video_view(display_frame,1);
