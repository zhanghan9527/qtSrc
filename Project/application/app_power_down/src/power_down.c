...
void system_speed_set_48MHz(void)
{
...
// mode 
// 0: sleep mode 
// 1: halt next instruction after wake up
...
{	
	...
	*P_USBD_CONFIG1 |= 0x100;		//[8],SW Suspend For PHY 
...
#elif 0	//rtc ALMEN wake up
	{
		...
		// must set current time before use RTC alarm.
		R_RTC_CTRL &= ~(0x07 << 8);
...
#else //idp rtc ALM pin wake up
	{
		...
		idp_rtc_int_set(GPX_RTC_DAY_IEN, 0);	//disable day int
	}
#endif	
#endif	//RTC_WAKEUP
	...
	*P_SPU_CONTROL_FLAG &= ~BIT11;
	...
	#endif
	#if (RTC_WAKEUP == 1)
		while(R_RTC_BUSY & (0xE000));	// wait RTC busy
	#endif
		R_SYSTEM_HALT = 0x500A;			
	} else if(mode==4) { 
		//halt mode and wake up reset				
	#if (defined MCU_VERSION) && (MCU_VERSION >= GPL327XX)
		system_speed_set_48MHz();		//set system to 48MHz
		HALT_MODE_RESET_VECTOR = 0; 	//set halt mode reset address
	#endif				
		R_SYSTEM_CTRL |= (1 << 0);		//let the 6MHz XTAL PAD enter sleep mode when system is in Halt mode.	
		R_SYSTEM_PLL_WAIT_CLK = 0x100; 	//set pll wait clock to 8 ms when wakeup/	
    ...
    //disable auto enter self refresh mode
    if(mode==1) {
    	sys_sdram_auto_refresh_set(0);	
	}
}