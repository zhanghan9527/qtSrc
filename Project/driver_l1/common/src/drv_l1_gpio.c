...
/*
Function Name	gpio_set_memA12_A13_A14
Description	Set IOG[9:7] as CS pins or GPIOs
Header File	drv_l1_gpio.h
Syntax	BOOLEAN gpio_set_memA12_A13_A14(INT8U addr_port,INT8U addr_io)
Parameters	addr_port: Address pin number(1~3 for A[12:14])
			addr_io: 1: Address pin, 0: GPIO
Return Values	0: Success 1: Fail
*/
#if (defined MCU_VERSION) && ((MCU_VERSION >= GPL327XX) || (MCU_VERSION < GPL326XXB))
BOOLEAN gpio_set_memA12_A13_A14(INT8U addr_port,INT8U addr_io)
{
    INT32U memset;

    if((addr_port>14) && (addr_port <12))
    	return 1;

    memset = 1 << (addr_port - 5);  
    if (addr_io) //Address
    {
      R_MEM_IO_CTRL |= memset;
    }
...
/*
Function Name	gpio_set_memA19_A20_A21_A22
Description	Set IOG[5:6:10:11] as Address pins or GPIOs
Header File	drv_l1_gpio.h
Syntax	BOOLEAN gpio_set_memA19_A20_A21_A22(INT8U addr_port,INT8U addr_io)
Parameters	addr_port: Address pin number(22~19 for A[22:19])
			addr_io: 1: Address pin, 0: GPIO
Return Values	0: Success	1: Fail
*/
#if (defined MCU_VERSION) && (MCU_VERSION < GPL327XX) 
BOOLEAN gpio_set_memA19_A20_A21_A22(INT8U addr_port,INT8U addr_io)
{
    INT32U memset;

    if((addr_port>22) && (addr_port <19))
    	return 1;
    
    memset = 1 << (addr_port - 18);  
    if (addr_io) //Address
    {
      R_MEM_IO_CTRL |= memset;
    }
...
void gpio_funpos_check(void)
{
	...
	INT32U iosel = R_IOSRSEL;
	INT32U fun0 = R_FUNPOS0;
	INT32U fun1 = R_FUNPOS1;
	INT32U fun2 = R_FUNPOS2;
	INT32U fun3 = R_FUNPOS3;
	
	//UART TX/RX Pins Selection 
	if(iosel&0x8000){	//{0xC0000104[15], 0xC0000108[0]}
		if(fun0&0x0001)
			DBG_PRINT("UART TX/RX on IOF3/ IOF8 \r\n");	
		else
			DBG_PRINT("UART TX/RX on IOB2/ IOB1 \r\n");
	} else{
		if(fun0&0x0001)
			DBG_PRINT("UART TX/RX on IOH3/ IOH2 \r\n");
		else
			DBG_PRINT("UART TX/RX on IOE2/ IOE3 \r\n");
	}

	if((fun3&0x1000)&&(fun3&0x0060)){
		DBG_PRINT("NF I/F on IOI[9:0] + IOF[7:6]");
	}else{
		//none-shared NF_ALE/CLE/REBWEB Pins Selection
		if(iosel&0x4000){	
			DBG_PRINT("NF_ALE/CLE/REBWEB on IOG5/6/10/11 \r\n");
			
		}else{
			if(fun0&0x0008)
				DBG_PRINT("NF_ALE/CLE/REBWEB on IOC6/7/8/9 \r\n");
		}
		
		//none-shared NF Data Pins Selection
		
		if(R_SYSMONI_CTRL&0x0080){
			DBG_PRINT("NF Data Pins[7:0] on IOA[15:8] \r\n");
		}else{
			if(!(fun0&0x03C0))
				DBG_PRINT("NF Data[7:0] Pins on IOB[15:8] \r\n");
		}
	...
	//SDC Pins Selection 
	if(fun2&0x0200){
		DBG_PRINT("SDC0(CMD/CLK/D0/D1/D2/D3 on IOB8/9/11/12/13/10 \r\n ");
	}else{
		if(!(iosel&0x0800))
			DBG_PRINT("SDC0(CMD/CLK/D0/D1/D2/D3 on IOC4/5/7/8/9/6 \r\n ");
	}	
	
	//CMOS Sensor Pins Selection
	if(fun0&0x8000){
		if(fun1&0x0002)
			DBG_PRINT("CSI_Data7_0/HSYNC/VSYNC/CLKI_CLK0 on IOA15:8/IOB5/IOB6/IOC10/IOH0 \r\n ");
		else
			DBG_PRINT("CSI_Data7_0/HSYNC/VSYNC/CLKI_CLK0 on IOA15:8/IOB5/IOB6/IOC10/IOC11 \r\n ");	
	}else{
		if(!(fun0&0x0400))
			DBG_PRINT("CSI_Data7_0/HSYNC/VSYNC/CLKI_CLK0 on IOA7:0/IOB3/IOB2/IOB1/IOB0 \r\n ");	
		else{
			if(fun2&0x0010)
				DBG_PRINT("CSI_Data7_0/HSYNC/VSYNC/CLKI_CLK0 on IOB15:8/IOB4/IOB7/IOF3/IOH0 \r\n ");
		}
				
	}
	
	//Memory Address[22:21] Pins Selection Bit	
	if(fun0&0x4000)
		DBG_PRINT("MEM_A22_A21 on IOG11_IOG10 \r\n ");
		
	
	if(fun3&0x0800)
		DBG_PRINT("KeyChangeC on IOF15 \r\n ");
	if(!(fun0&0x1000)&& (fun3&0x0400))
		DBG_PRINT("KeyChangeB on IOF3 \r\n ");
	if(fun3&0x0200)
		DBG_PRINT("KeyChangeA on IOF2 \r\n ");
				
	//TFT HSYNC/VSYNC/CLK Pins Selection Bit	
	if(!(fun0&0x0002) && !(fun1&0x0040))
		DBG_PRINT("TFT HSYNC_VSYNC_CLK Pins on IOB1_IOB2_IOB3 \r\n ");
	if(!(fun1&0x0040))
		DBG_PRINT("TFT DE Pin on IOB0 \r\n ");

	if(!(fun1&0x0010))
		DBG_PRINT("TFT Data0 Pin on IOA0 \r\n ");
		
	if(!(fun2&0x0100))
		DBG_PRINT("TFT Data1 Pin on IOA1 \r\n ");
	else
		DBG_PRINT("IOA1 as GPIO \r\n");

	if(fun3&0x0100)
		DBG_PRINT("TFT DE Pin on IOG14 \r\n ");
	else
		DBG_PRINT("IOG14 could be GPIO \r\n");			
	
	//SDRAM CKE Pin PortH Output Enable Bit
	if(!(fun1&0x0020))
		DBG_PRINT("SDRAM Pin on IOH1 \r\n ");	
	
	//Memory Output Enable Pin PortH Output Enable Bit	
	if(!(fun1&0x0004))
		DBG_PRINT("Memory OE Pin on IOH0 \r\n ");	
	
	//JTAG Pins Enable Bit
	if(fun1&0x0001)
		DBG_PRINT("IOH[5:2] as JTAG Pins\r\n ");		
	else
		DBG_PRINT("IOH[5:2] as GPIOs\r\n ");
	
	//DCDC PWM0&1 Pins Selection	
	if(R_PWM_CTRL&0x3800)
		DBG_PRINT("IOG[15:13] as DCDC PWM1 analog signals\r\n ");	
	else
		DBG_PRINT("IOG[15:13] as GPIOs\r\n ");		

	if(R_PWM_CTRL&0x0300)
		DBG_PRINT("IOG[11:10] as DCDC PWM0 analog signals\r\n ");	
	else
		DBG_PRINT("IOG[11:10] as GPIOs\r\n ");		

	if(fun3&0x0020)
		DBG_PRINT("EXTB on IOH7 \r\n");
	else
		DBG_PRINT("EXTB on IOC10 \r\n");
	
	//SDC1 I/F 	
	if((fun3&0x0004)&&!(fun3&0x0080))
		DBG_PRINT("IOI[9:4] as SDC1 I/F \r\n");			
	//SPI1 I/F				
	if((fun3&0x0002)&&!(fun3&0x0080))
		DBG_PRINT("IOI[3:0] as SPI1 I/F \r\n");			

	if(fun3&0x0001)
		DBG_PRINT("IOG14 is SPI0 CS pin for slave mode\r\n");			
	else
		DBG_PRINT("SPI0 CS pin for slave mode is tied to DVDD33 internally \r\n");
#endif		
}
