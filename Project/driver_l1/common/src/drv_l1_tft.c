...
void HV430V3B_HX8257_init(void)
{	/* 480RGB*272 */

/*	R_TFT_V_START = 12;
	R_TFT_V_END	= 12+272-1;
	R_TFT_V_PERIOD = 288;	
	

	R_TFT_H_START = 43;
	R_TFT_H_END	= 43+480-1;
	R_TFT_H_PERIOD = 531;	
	...
	R_TFT_TS_MISC = 1;  */
        ...
        R_TFT_V_START         = 2;//12;                                        //=VPW+VBP                        (LINE) 
        R_TFT_V_END                = 2+272-1;//12+272-1;                         //=VPW+VBP+VDE                (LINE) 
        R_TFT_V_PERIOD        = 2+272+12; //288;                                //=VPW+VBP+VDE+VFP        (LINE) 
        

        R_TFT_H_START         = 2;//43;                                 //=HPW+HBP 
        R_TFT_H_END                = 2+480-1;//43+480-1;                                //=HPW+HBP+HDE 
        R_TFT_H_PERIOD  = 2+480+43;//531;                                //=HPW+HBP+HDE+HFP 
...
//        R_TFT_HS_END = 0; 
        ...
        R_TFT_VS_WIDTH = 10;                                //=VPW                                (DCLK) 
        R_TFT_HS_WIDTH = 41;                                        //=HPW 
        
        R_TFT_TS_MISC = 1; 
	
	
	tft_rb_switch_set(TRUE);
	tft_data_mode_set(TFT_DATA_MODE_565);
	tft_gamma_en_set(TRUE);
...
// 2014.06.20
void FT043S480272NC07_init(void)
{	/* 480RGB*272 */
...
/*	R_TFT_V_START = 12;
	...
	R_TFT_TS_MISC = 1;  */
        ...
        R_TFT_V_START         = 1+1;//12;                                     //=VPW+VFP                (LINE) 
        R_TFT_V_END           = 1+1+272;//12+272-1;                    		//=VPW+VFP+VDE            (LINE) 
        R_TFT_V_PERIOD        = 1+1+272+12; //288;                        	//=VPW+VFP+VDE+VBP        (LINE) 
...
//*****************************************************************************
//
//! Reset the 8080-Controller TFT LCD.
//
//*****************************************************************************
#define  T80_RESET_PIN 	IO_A8		//IO_B4
static void reset(void)
{
    gpio_set_port_attribute(T80_RESET_PIN, ATTRIBUTE_HIGH); 
    gpio_init_io(T80_RESET_PIN, GPIO_OUTPUT); 
    gpio_write_io(T80_RESET_PIN, DATA_HIGH); 
    drv_msec_wait(10);
    gpio_write_io (T80_RESET_PIN, DATA_LOW); 
    drv_msec_wait(20);
    gpio_write_io(T80_RESET_PIN, DATA_HIGH);
    drv_msec_wait(20);
}

void tft_i80_write_cmd(INT8U cmd)//(INT32U cmd)
{
	R_TFT_MEM_BUFF_WR = cmd; 
	R_TFT_CTRL        = 0x2080 | ((4 << 1) | 1);
	/*
	INT32U reg = R_TFT_CTRL;
	
	if(reg & TFT_MEM_BYTE_EN) { //byte mode
		R_TFT_MEM_BUFF_WR = cmd;
	} else {
		R_TFT_MEM_BUFF_WR = cmd & 0xFFFF;
	}
	
	reg &= ~TFT_MODE;
	reg |= TFT_MODE_MEM_CMD_WR;
	reg |= TFT_EN;
	R_TFT_CTRL = reg;
	*/
}

void tft_i80_write_data(INT8U data)//(INT32U data)
{
	R_TFT_MEM_BUFF_WR = data; 
	R_TFT_CTRL        = 0x20a0 | ((4 << 1) | 1);
/*
	INT32U reg = R_TFT_CTRL;
	...
	if (reg & TFT_MEM_BYTE_EN) { //byte mode
		...
		R_TFT_MEM_BUFF_WR = data & 0xFFFF;
	}
	
	reg &= ~TFT_MODE;
	reg |= TFT_MODE_MEM_DATA_WR;
	reg |= TFT_EN;
	R_TFT_CTRL = reg;
*/
}
...
#define RGB_GREY				0x959C  /* 1001 0101 1001 1100 */
#define RGB_GREEN				0x07E0  /* 0000 0111 1110 0000 */
#define RGB_BLACK				0x0000  /* 0000 0000 0000 0000 */


void JC14402_ST7735S_8bit_init(void)
{	/* 128*128 */
//  2014.09.18  点亮  
	long int i,j;
	...
	reset();
/*
    R_TFT_VS_WIDTH = 5;                                			//=VPW                     (DCLK) 
    R_TFT_V_START         = 5+3;                        			//=VPW+VBP                      (LINE) 
    R_TFT_V_END           = 5+3+128-1;       						//=VPW+VBP+VDE                	(LINE) 
    R_TFT_V_PERIOD        = 5+3+128-1; //288               		//=VPW+VBP+VDE+VFP        		(LINE) 
    

    R_TFT_HS_WIDTH = 20;                                    			//=HPW 
    R_TFT_H_START         	= 20+5;//43;                       	//=HPW+HBP 
    R_TFT_H_END             = 20+5+128-1;//43+480-1;       		//=HPW+HBP+HDE 
    R_TFT_H_PERIOD  		= 20+5+128-1;//531;               	//=HPW+HBP+HDE+HFP 

  
    R_TFT_TS_MISC = 0x0101; 
   
	

//	tft_mem_csb_mode_set(TRUE);	//memory mode
	R_TFT_LINE_RGB_ORDER |= TFT_NEW_POL_EN;		
	
	tft_mem_unit_set(1); 		//Byte size
	tft_data_mode_set(TFT_DATA_MODE_565);
*/                        
	R_TFT_CTRL = 0;
    ...
    R_TFT_VS_END   = 128-1; 
    R_TFT_TS_MISC  = 0x0101; 
    R_TFT_LINE_RGB_ORDER |= 0x0080;
	
	
//	tft_rb_switch_set(TRUE);

	
//	tft_vsync_unit_set(TRUE);			//Vsync  unit base on Hsync
//	tft_signal_inv_set(TFT_VSYNC_INV|TFT_HSYNC_INV, (TFT_ENABLE & TFT_VSYNC_INV)|(TFT_ENABLE & TFT_HSYNC_INV));


	tft_i80_write_cmd(0X01);		//SOFTWARE RESET
    drv_msec_wait(200);
	tft_i80_write_cmd(0X11);		//sleep out
    drv_msec_wait(200);
	
//	tft_i80_write_cmd(0X04);
	

	// ST7735S Frame Rate //
	tft_i80_write_cmd(0xB1);     	//Frame rate=fosc/((RTNA*2+40) * (LINE+FPA+BPA+2)) / fosc=850KHz
	tft_i80_write_data(0x05);     		//frq=850/((5*2+40) * (132+58+58+2) =81.5Hz
	tft_i80_write_data(0x3a);     		//
	tft_i80_write_data(0x3a);     		//

	tft_i80_write_cmd(0xB2);     	//Frame rate 80Hz
	tft_i80_write_data(0x05);
	tft_i80_write_data(0x3a);
	tft_i80_write_data(0x3a);

	tft_i80_write_cmd(0xB3);     	//Frame rate 80Hz
	tft_i80_write_data(0x05);
	tft_i80_write_data(0x3a);
	tft_i80_write_data(0x3a);
	tft_i80_write_data(0x05);
	...
	tft_i80_write_cmd(0xB4); 		//Display Inversion mode
	tft_i80_write_data(0x02);     		//0x00:Dot inversion //0x07:Column inversion

	// ST7735S Power Sequence 	//
	tft_i80_write_cmd(0xC0);    	 	// ctrv =0x02
	tft_i80_write_data(0x28);			//AVDD[2:0]=001: AVDD=5.0V // VRHP[4:0]=00010 :GVDD=4.6V
	tft_i80_write_data(0x08);			//VRHN[4:0]=00010: GVCL= -4.6V
	tft_i80_write_data(0x84);			//MODE[1:0]=D[7:6]=10: AUTO 

	tft_i80_write_cmd(0xC1);
	tft_i80_write_data(0xC0);

	tft_i80_write_cmd(0xC2);
	tft_i80_write_data(0x0D);
	tft_i80_write_data(0x00);

	tft_i80_write_cmd(0xC3);
	tft_i80_write_data(0x8D);
	tft_i80_write_data(0x2A);

	tft_i80_write_cmd(0xC4);
	tft_i80_write_data(0x8D);
	tft_i80_write_data(0xEE);
	//---------------------------------End ST7735s Power Sequence-------------------------------------//
	tft_i80_write_cmd(0xC5); 	//VCOM
	tft_i80_write_data(0x0c);

	tft_i80_write_cmd(0x36); 	//MX, MY, RGB mode
	//tft_i80_write_data(0x00);
	tft_i80_write_data(0x08);		//bit5  水平垂直切换  bit6  水平反相 bit7 垂直反相60

	//------------------------------------ST7735S Gamma Sequence-----------------------------------------// 
	//*
	tft_i80_write_cmd(0xe0);
	tft_i80_write_data(0x05);    //VRF0P[5:0]: Variable resistor VRHP /High level adjustment
	tft_i80_write_data(0x1a);    //VOS0P[5:0]: Variable resistor VRLP /Low level adjustment
	tft_i80_write_data(0x0c);    //Mid level adjustment
	tft_i80_write_data(0x0e);
	tft_i80_write_data(0x3a);
	tft_i80_write_data(0x34);
	tft_i80_write_data(0x2d);
	tft_i80_write_data(0x2f);
	tft_i80_write_data(0x2d);
	tft_i80_write_data(0x2a);
	tft_i80_write_data(0x2f);
	tft_i80_write_data(0x3c);
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x01);
	tft_i80_write_data(0x02);
	tft_i80_write_data(0x10);

	tft_i80_write_cmd(0xe1);
	tft_i80_write_data(0x04);
	tft_i80_write_data(0x1b);
	tft_i80_write_data(0x0d);
	tft_i80_write_data(0x0e);
	tft_i80_write_data(0x2d);
	tft_i80_write_data(0x29);
	tft_i80_write_data(0x24);
	tft_i80_write_data(0x29);
	tft_i80_write_data(0x28);
	tft_i80_write_data(0x26);
	tft_i80_write_data(0x31);
	tft_i80_write_data(0x3b);
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x03);
	tft_i80_write_data(0x12);
	//*/
	
	tft_i80_write_cmd(0x2a);   
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);//0x02 03
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x80);//0x81 82

	tft_i80_write_cmd(0x2b);   
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);//0x03 when 0x36h is 0xc8 for 7735r02
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x80);//0x82 when 0x36 is 0xc8  for 7735r 81
	
	//------------------------------------End ST7735S Gamma Sequence-----------------------------------------//

	tft_i80_write_cmd(0xFC); 		//
	tft_i80_write_data(0x8c);

	tft_i80_write_cmd(0x3A); 		//65k mode
	tft_i80_write_data(0x05);
    
    tft_i80_write_cmd(0x29);		 /* Display on */
  	/* Clear RAM*/
	tft_i80_write_cmd(0X2C);

	for(i=0;i<128;i++)
	{
	   for(j=0;j<128;j++)
	   {
			tft_i80_write_data(RGB_GREEN>>8);
			tft_i80_write_data((INT8U)RGB_GREEN);
	   }
	} 
...
//	cmd_longdelay(3000);			
	tft_i80_write_cmd(0x29);		 /* Display on */
	cmd_longdelay(3000);		
	tft_i80_write_cmd(0x2c);		 //write dispaly data command
	cmd_longdelay(3000);	

//	#if I80_MODE == I80_CONTI
//	tft_mode_set(TFT_MODE_MEM_CONTI);
//	tft_en_set(1);
//	#elif I80_MODE == I80_ONCE	
//	tft_mode_set(TFT_MODE_MEM_ONCE);
//	#endif	
...
void JMP17009_ST7735S_8bit_init(void)
{	/* 128*128 */
//  2014.09.18  点亮  
	long int i,j;
...
//	tft_i80_write_cmd(0X04);
	...
	tft_i80_write_cmd(0xB4); 		//Display Inversion mode
	tft_i80_write_data(0x03);     		//0x00:Dot inversion //0x07:Column inversion
	...
	tft_i80_write_cmd(0xC2);
	tft_i80_write_data(0x0C);	//)0x0D
	tft_i80_write_data(0x00);

	tft_i80_write_cmd(0xC3);
	tft_i80_write_data(0x8C);	//8D
	tft_i80_write_data(0x2A);
	...
	tft_i80_write_cmd(0x36); 	//MX, MY, RGB mode
	//tft_i80_write_data(0x00);
	tft_i80_write_data(0xC8);		//bit5  水平垂直切换  bit6  水平反相 bit7 垂直反相
	
	//tft_i80_write_cmd(0x37); 	
	//tft_i80_write_data(0x00);
	//tft_i80_write_data(0x00);	
	...
	tft_i80_write_cmd(0x2a);   
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);//0x02
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x7F);//0x81

	tft_i80_write_cmd(0x2b);   //Virtual
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x20);//0x03 when 0x36h is 0xc8 for 7735r
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x9F);//0x82 when 0x36 is 0xc8  for 7735r 
	...
	/*for(i=0;i<162;i++)
	{
	   for(j=0;j<132;j++)
	   {
			tft_i80_write_data(RGB_BLACK>>8);
			tft_i80_write_data((INT8U)RGB_BLACK);
	   }
	} //*/
...
void JMP17009_ST7735S_8bit_init2(void)
{	/* 128*128 */
//  2014.09.18  点亮  
	long int i,j;
...
//	tft_i80_write_cmd(0X04);
	...
	tft_i80_write_cmd(0x2a);   
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x01);//0x02
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x80);//0x81

	tft_i80_write_cmd(0x2b);   
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x02);//0x03 when 0x36h is 0xc8 for 7735r
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x81);//0x82 when 0x36 is 0xc8  for 7735r
...
//void TXDT240NQ_ST7789_I80_16(void)		//驱动与HSM240T004_36A_7789S_I80_16(void) 一样

void HSM240T004_36A_7789S_I80_16(void)
{	
	/* 240RGB*320 	2015.05.08 OK */

	long int i,j;
	

    *(int *)IOA_DIR_ADDR |=0xFFFF;
    *(int *)IOA_ATTRIB_ADDR |=0xFFFF; 
    *(int *)IOA_DATA_ADDR |=0xFFFF;
    *(int *)IOA_BUFFER_ADDR =0;        
...
//	tft_i80_write_cmd(0X01);		//SOFTWARE RESET
//	cmd_longdelay(1000);
	tft_i80_write_cmd(0X11);
//	cmd_longdelay(1000);
	drv_msec_wait(10);	
//	tft_i80_write_cmd(0X29);
	
//	ILI9341_RD_REG(0XD3,4);		//READ ID
//	temp1=DATA[0];
//	temp2=DATA[1];
//	temp3=DATA[2];
//	temp4=DATA[3];			
...
//--------------------------------------Display Setting------------------------------------------//
	...
	tft_i80_write_cmd(0X36);
	tft_i80_write_data(0x60);			//bit5  横  竖切换
				
				
	tft_i80_write_cmd(0X3a);
	tft_i80_write_data(0x55);					
//--------------------------------ST7789V Frame rate setting----------------------------------//
	tft_i80_write_cmd(0XB2);			//Power Control A
	tft_i80_write_data(0x0c);
	tft_i80_write_data(0x0c);
	tft_i80_write_data(0x00);	
	tft_i80_write_data(0x33);	
	tft_i80_write_data(0x33);		
				
				
	tft_i80_write_cmd(0Xb7);			
	tft_i80_write_data(0x33);				
//---------------------------------ST7789V Power setting--------------------------------------//
	tft_i80_write_cmd(0Xbb);				
	tft_i80_write_data(0x33);				

	tft_i80_write_cmd(0Xc0);
	tft_i80_write_data(0x2c);				
				
	tft_i80_write_cmd(0Xc2);
	tft_i80_write_data(0x01);

	tft_i80_write_cmd(0Xc3);
	tft_i80_write_data(0x08);

	tft_i80_write_cmd(0Xc4);
	tft_i80_write_data(0x20);

	tft_i80_write_cmd(0Xc6);
	tft_i80_write_data(0x0F);

	tft_i80_write_cmd(0XD0);
	tft_i80_write_data(0xA4);
	tft_i80_write_data(0xA1);	
//--------------------------------ST7789V gamma setting---------------------------------------//				
				
	tft_i80_write_cmd(0X2A);		//
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);					
	tft_i80_write_data(0x01);		//01
	tft_i80_write_data(0x3F);		//3F				
				
	tft_i80_write_cmd(0X2B);			
	tft_i80_write_data(0x00);
	tft_i80_write_data(0x00);		//00			
	tft_i80_write_data(0x00);
	tft_i80_write_data(0xEF);		//EF				
	...
	tft_i80_write_cmd(0XE0);
	tft_i80_write_data(0xD0);
	tft_i80_write_data(0x00);					
	tft_i80_write_data(0x04);
	tft_i80_write_data(0x0e);
	tft_i80_write_data(0x15);
	tft_i80_write_data(0x0d);					
	tft_i80_write_data(0x30);
	tft_i80_write_data(0x54);
	tft_i80_write_data(0x43);
	tft_i80_write_data(0x07);					
	tft_i80_write_data(0x12);
	tft_i80_write_data(0x12);
	tft_i80_write_data(0x12);	
	tft_i80_write_data(0x19);		
	...
	tft_i80_write_data(0x0d);
	tft_i80_write_data(0x0c);
	tft_i80_write_data(0x06);					
	tft_i80_write_data(0x29);
	tft_i80_write_data(0x55);
	tft_i80_write_data(0x3c);
	tft_i80_write_data(0x0c);					
	tft_i80_write_data(0x19);
	tft_i80_write_data(0x18);
	tft_i80_write_data(0x12);	
	tft_i80_write_data(0x19);	
...
//*/	
  ...
  //  cmd_longdelay(50);
	...
	tft_mode_set(TFT_MODE_MEM_CONTI);
   	...
   	if (MHZ == 96)
    {
    	tft_clk_set(TFT_CLK_DIVIDE_2);/* FS = 60 */
    }
    else
    {
    	tft_clk_set(TFT_CLK_DIVIDE_4);
